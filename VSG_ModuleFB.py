...
VSGVersion='VSG2FB_05_29_23'
## PDF, LegendInfo

## For Current Version of VSG_Module and Documentation, email afire@stanford.edu
## Recent versions will also be posted on https://github.com/FireLabSoftware/VSG_Module
## version DA fixes a bug that duplicated the colorkey in some cases.
## version DC fixes a bug that resulted from improper list conversion for Python 3.8.
## version DD fixes a number of bugs in vcolorkey (12/21/21)
## version EG adds a primative set of standard interface tools: vCommand, vOpen, vClose, and vLog
##   importing VSG after a block of constant defiitions as "from VSG_Module## import *"
##   takes all previous assignment statements (eg, x=1) and allows variables to be reset from the command line
##       the keyword vcommand just needs to be added to the command line before assignments
##       e.g. python myscript.py 2 4 vcommand myoutputfile='newfile.txt'
##       will pass the first two command line arguments to the script as normal arguments
##       while the last assignment myoutputfile='newfile.txt' will reassign the variable myoutputfile
##   vOpen("FileName", mode='xx') and vClose(FileObject) will open/close files with standard tools or Zip/Gzip 
##   vLog("MyNote") opens up a time-stamped log file and writes the note+time to this file
##   vFastAToDict(FastAFileName).  Returns a dictionary where keys are sequence names and values are sequences
##   vtdvToDict(TabDelimitedValueFileName).  Returns a list of headers (text list) and a dictionary where
##     keys are row ids (tuple taken from the first few columns in each row)
##     values are lists of column entries for the individual rows (converted to integer if possible, float, or kept as text)
##   vantisense(Sequence).  Provides antisense sequence for any input DNA
## Version EO fixes a legacy deprecation of PILFont.getsize with the preferred alternatives .getbbox and .getlength
##   Additionally a function and command line option is added to convert files from svg to pdf.
##     Syntax python VSG_Module##.py vRenderPDF (can add a file or wildcard to instruct VSG which files to convert)
##     Default for VSG_Module##.py vRenderPDF is to convert all *.svg files in current directory
## Version EU adds an ability to specify strokewidths in a manner that ensures constant size as a drawing is zoomed in SVG
##   syntax examples: vcircle(r=4,stroke=red,strokewidth=2,svgstrokezoom=True) [just that object has a fixed stroke]
##     vset(svgstrokezoom=True) [use a fixed stroke for all subsequently defined objects]
## Version EZ fixes a syntax bug related to legacy python (Python 2)
import gzip, zipfile, os, sys, ast
from time import time, strftime, localtime
from itertools import chain
from math import * ## sin, cos, sqrt, log, log2, log10, floor, ceil, trunc, pi
...
def modification_date(filename):
    ...
    return str(datetime.datetime.fromtimestamp(t))[::-1].split('.',1)[-1][::-1]

def textrip(arglist):
    s1 = ''
    arglist2 = sum([('_'+arg).split('=',1) for arg in arglist], [])
    for arg in arglist2:
        if ('/' in arg) or (':' in arg):
            try:
                if os.path.basename(arg):
                    ...
                    arg = os.path.basename(arg[:-1]) ## for a pathlist just take the last element
            except:
                pass
        if '.' in arg:
            p = arg.rfind('.')
            arg0 = arg[:p]
            arg1 = arg[p+1:]
            if arg0 and arg1.isalnum() and not(arg1.isdigit()):
                arg = arg0
        for c in arg:
            if s1 and c=='_' and s1[-1]=='-':
                s1 = s1[:-1]
            if c.isalnum() or c in '.-_':
                ...
                s1+='x'
        if s1[-1]!='-' and s1[-1]!='_' and arg[0]=='_':
            s1+='-'
    if not(s1):
        return ''
    while s1[-1] in '._-':
...
vshortnow = strftime("%m%d%y-%H%M",localtime())
DefaultFileBase = 'vsgOut_'+textrip(sys.argv)+'_'+vshortnow
DefaultFileBase = DefaultFileBase[:242]
vSysLogInfo1 = 'VSG Run Details:\n'
vSysLogInfo1 += '  Python Interpreter: '+' '.join(sys.version.split('\n'))+'\n'
vSysLogInfo1 += '  VSG Version: '+VSGVersion+' '+ strftime(" Run at %H:%M on %m/%d/%y\n",localtime())
if len(sys.argv)>0 and os.path.isfile(sys.argv[0]):
    vSysLogInfo1 += '  Script: '+os.path.basename(sys.argv[0])+' (modified '+ modification_date(sys.argv[0])+')\n'
    if os.path.dirname(sys.argv[0]):
        vSysLogInfo1+='    Script Directory: '+os.path.dirname(sys.argv[0])+'\n'
    else:
        vSysLogInfo1+='    Script Directory: '+os.getcwd()+'\n'
    if len(sys.argv)>1:
        vSysLogInfo1 += '  Parameters:\n'
        for item1 in sys.argv[1:]:
            vSysLogInfo1 += '    '+item1
            if item1:
                possiblefilename1 = item1.split('=')[-1]
                if os.path.isfile(possiblefilename1):
                    vSysLogInfo1 += ' (modified '+modification_date(possiblefilename1)+')'  ## fixed improper file name reference 5-11-23
            vSysLogInfo1 += '\n'
else:
    vSysLogInfo1 = '  Script: Python From Console \n'
vDefaultLogFileName = 'VSGLog_'+vnow+os.path.basename(sys.argv[0]).split('.')[0]+'.txt'
...
## VSG2 Copywrite 2009-2023 Andrew Fire and Stanford University.  All rights reserved
## Send bug reports to afire<rat>stanford.edu (changing the nice rodent into an @ sign)

## VSG needs to have the correct path if it is to find GraphicsMagick/ImageMagick...
## this may need to be be revisited with other installations of GM/IM or with other systems.
## the issue here is that some IDEs doesn't use the same PATH options as the terminal
## this can make it difficult to call programs that have been installed for use at the command
## line in terminal.  The list below was pulled out 3/2022 from a MAC snow leopard terminal
## using the command echo $PATH in terminal.  Thanks for the explanation of this go to some guy
## who saw the code over my shoulder on a plane in 2011 and said he worked for Google but wouldn't
## tell me his name.
PathList1 = ':'.join(['/opt/local/bin','/opt/local/sbin','/usr/bin','/bin','/usr/sbin','/sbin','/usr/local/bin','/usr/X11/bin','/opt/X11/bin','/Library/Apple/usr/bin'])
os.environ["PATH"]=os.getenv("PATH")+':'+ PathList1
...
vhue={'yellow': 'rgb(255,255,0)', 'grey61': 'rgb(156,156,156)', 'grey60': 'rgb(153,153,153)',
'darkseagreen': 'rgb(143,188,143)', 'grey62': 'rgb(158,158,158)', 'grey65': 'rgb(166,166,166)',
'grey64': 'rgb(163,163,163)', 'grey67': 'rgb(171,171,171)', 'grey66': 'rgb(168,168,168)', 'grey69':
'rgb(176,176,176)', 'grey68': 'rgb(173,173,173)', 'gray32': 'rgb(82,82,82)', 'deepskyblue2':
'rgb(0,178,238)', 'gray30': 'rgb(77,77,77)', 'gray31': 'rgb(79,79,79)', 'gray36': 'rgb(92,92,92)',
'gray37': 'rgb(94,94,94)', 'gray34': 'rgb(87,87,87)', 'gray35': 'rgb(89,89,89)', 'aquamarine4':
'rgb(69,139,116)', 'gray38': 'rgb(97,97,97)', 'gray39': 'rgb(99,99,99)', 'aquamarine1':
'rgb(127,255,212)', 'aquamarine3': 'rgb(102,205,170)', 'aquamarine2': 'rgb(118,238,198)', 'crimson':
'rgb(220,20,60)', 'deepskyblue1': 'rgb(0,191,255)', 'brown': 'rgb(165,42,42)', 'coral3':
'rgb(205,91,69)', 'gray8': 'rgb(20,20,20)', 'gray9': 'rgb(23,23,23)', 'gray2': 'rgb(5,5,5)', 'cyan':
'rgb(0,255,255)', 'gray0': 'rgb(0,0,0)', 'gray1': 'rgb(3,3,3)', 'fractal': 'rgb(128,128,128)',
'gray7': 'rgb(18,18,18)', 'gray4': 'rgb(10,10,10)', 'gray5': 'rgb(13,13,13)', 'skyblue':
'rgb(135,206,235)', 'deepskyblue4': 'rgb(0,104,139)', 'rosybrown3': 'rgb(205,155,155)', 'grey63':
'rgb(161,161,161)', 'indianred1': 'rgb(255,106,106)', 'lightpink': 'rgb(255,182,193)', 'indianred2':
'rgb(238,99,99)', 'teal': 'rgb(0,128,128)', 'grey11': 'rgb(28,28,28)', 'lemonchiffon3':
'rgb(205,201,165)', 'lemonchiffon2': 'rgb(238,233,191)', 'lemonchiffon1': 'rgb(255,250,205)',
'bisque': 'rgb(255,228,196)', 'grey12': 'rgb(31,31,31)', 'lemonchiffon4': 'rgb(139,137,112)',
'gray99': 'rgb(252,252,252)', 'grey13': 'rgb(33,33,33)', 'grey14': 'rgb(36,36,36)',
'lightgoldenrodyellow': 'rgb(250,250,210)', 'lavender': 'rgb(230,230,250)', 'chartreuse3':
'rgb(102,205,0)', 'chartreuse2': 'rgb(118,238,0)', 'chartreuse1': 'rgb(127,255,0)', 'grey16':
'rgb(41,41,41)', 'chartreuse4': 'rgb(69,139,0)', 'grey17': 'rgb(43,43,43)', 'dimgrey':
'rgb(105,105,105)', 'blue': 'rgb(0,0,255)', 'opaque': 'rgb(0,0,0)', 'maroon4': 'rgb(139,28,98)',
'maroon3': 'rgb(205,41,144)', 'maroon2': 'rgb(238,48,167)', 'maroon1': 'rgb(255,52,179)', 'gold3':
'rgb(205,173,0)', 'gold2': 'rgb(238,201,0)', 'gold1': 'rgb(255,215,0)', 'gold4': 'rgb(139,117,0)',
'gray41': 'rgb(105,105,105)', 'gray33': 'rgb(84,84,84)', 'skyblue4': 'rgb(74,112,139)', 'skyblue1':
'rgb(135,206,255)', 'skyblue3': 'rgb(108,166,205)', 'skyblue2': 'rgb(126,192,238)', 'coral4':
'rgb(139,62,47)', 'lightskyblue4': 'rgb(96,123,139)', 'floralwhite': 'rgb(255,250,240)', 'gray60':
'rgb(153,153,153)', 'firebrick3': 'rgb(205,38,38)', 'firebrick2': 'rgb(238,44,44)', 'steelblue3':
'rgb(79,148,205)', 'steelblue2': 'rgb(92,172,238)', 'steelblue4': 'rgb(54,100,139)', 'firebrick4':
'rgb(139,26,26)', 'grey51': 'rgb(130,130,130)', 'aliceblue': 'rgb(240,248,255)', 'greenyellow':
'rgb(173,255,47)', 'lightseagreen': 'rgb(32,178,170)', 'sienna': 'rgb(160,82,45)', 'blue1':
'rgb(0,0,255)', 'lime': 'rgb(0,255,0)', 'goldenrod2': 'rgb(238,180,34)', 'royalblue':
'rgb(65,105,225)', 'gainsboro': 'rgb(220,220,220)', 'peru': 'rgb(205,133,63)', 'darkslategray':
'rgb(47,79,79)', 'red3': 'rgb(205,0,0)', 'red2': 'rgb(238,0,0)', 'red1': 'rgb(255,0,0)',
'dodgerblue': 'rgb(30,144,255)', 'red4': 'rgb(139,0,0)', 'lemonchiffon': 'rgb(255,250,205)',
'lightgreen': 'rgb(144,238,144)', 'darkgrey': 'rgb(169,169,169)', 'olive': 'rgb(128,128,0)',
'antiquewhite': 'rgb(250,235,215)', 'grey18': 'rgb(46,46,46)', 'grey19': 'rgb(48,48,48)',
'moccasin': 'rgb(255,228,181)', 'grey10': 'rgb(26,26,26)', 'chocolate1': 'rgb(255,127,36)',
'chocolate2': 'rgb(238,118,33)', 'chocolate3': 'rgb(205,102,29)', 'chocolate4': 'rgb(139,69,19)',
'grey15': 'rgb(38,38,38)', 'darkslateblue': 'rgb(72,61,139)', 'lightskyblue': 'rgb(135,206,250)',
'gray69': 'rgb(176,176,176)', 'gray68': 'rgb(173,173,173)', 'deeppink': 'rgb(255,20,147)', 'gray65':
'rgb(166,166,166)', 'gray64': 'rgb(163,163,163)', 'gray67': 'rgb(171,171,171)', 'gray66':
'rgb(168,168,168)', 'gray61': 'rgb(156,156,156)', 'coral': 'rgb(255,127,80)', 'gray63':
'rgb(161,161,161)', 'gray62': 'rgb(158,158,158)', 'springgreen1': 'rgb(0,255,127)', 'springgreen2':
'rgb(0,238,118)', 'springgreen3': 'rgb(0,205,102)', 'springgreen4': 'rgb(0,139,69)', 'lightgray':
'rgb(211,211,211)', 'seashell2': 'rgb(238,229,222)', 'seashell3': 'rgb(205,197,191)', 'magenta':
'rgb(255,0,255)', 'seashell1': 'rgb(255,245,238)', 'tan': 'rgb(210,180,140)', 'seashell4':
'rgb(139,134,130)', 'pink': 'rgb(255,192,203)', 'palevioletred': 'rgb(219,112,147)', 'powderblue':
'rgb(176,224,230)', 'mediumblue': 'rgb(0,0,205)', 'grey89': 'rgb(227,227,227)', 'gray100':
'rgb(255,255,255)', 'grey87': 'rgb(222,222,222)', 'grey86': 'rgb(219,219,219)', 'grey85':
'rgb(217,217,217)', 'grey84': 'rgb(214,214,214)', 'midnightblue': 'rgb(25,25,112)', 'grey82':
'rgb(209,209,209)', 'grey81': 'rgb(207,207,207)', 'grey80': 'rgb(204,204,204)', 'gray40':
'rgb(102,102,102)', 'lightslategray': 'rgb(119,136,153)', 'dodgerblue1': 'rgb(30,144,255)',
'mediumpurple1': 'rgb(171,130,255)', 'mediumpurple2': 'rgb(159,121,238)', 'mediumpurple3':
'rgb(137,104,205)', 'mediumpurple4': 'rgb(93,71,139)', 'violetred2': 'rgb(238,58,140)', 'khaki4':
'rgb(139,134,78)', 'paleturquoise4': 'rgb(102,139,139)', 'paleturquoise3': 'rgb(150,205,205)',
'khaki1': 'rgb(255,246,143)', 'khaki2': 'rgb(238,230,133)', 'khaki3': 'rgb(205,198,115)', 'salmon1':
'rgb(255,140,105)', 'honeydew4': 'rgb(131,139,131)', 'salmon3': 'rgb(205,112,84)', 'salmon2':
'rgb(238,130,98)', 'salmon4': 'rgb(139,76,57)', 'linen': 'rgb(250,240,230)', 'burlywood1':
'rgb(255,211,155)', 'green': 'rgb(0,128,0)', 'blueviolet': 'rgb(138,43,226)', 'brown2':
'rgb(238,59,59)', 'brown3': 'rgb(205,51,51)', 'brown1': 'rgb(255,64,64)', 'brown4':
'rgb(139,35,35)', 'orange4': 'rgb(139,90,0)', 'orange1': 'rgb(255,165,0)', 'orange3':
'rgb(205,133,0)', 'orange2': 'rgb(238,154,0)', 'slategrey': 'rgb(112,128,144)', 'gray3':
'rgb(8,8,8)', 'darkslategrey': 'rgb(47,79,79)', 'gray6': 'rgb(15,15,15)', 'yellow4':
'rgb(139,139,0)', 'yellow3': 'rgb(205,205,0)', 'yellow2': 'rgb(238,238,0)', 'yellow1':
'rgb(255,255,0)', 'orange': 'rgb(255,165,0)', 'grey88': 'rgb(224,224,224)', 'grey58':
'rgb(148,148,148)', 'grey59': 'rgb(150,150,150)', 'grey54': 'rgb(138,138,138)', 'mediumorchid4':
'rgb(122,55,139)', 'grey56': 'rgb(143,143,143)', 'grey57': 'rgb(145,145,145)', 'mediumorchid1':
'rgb(224,102,255)', 'silver': 'rgb(192,192,192)', 'mediumorchid3': 'rgb(180,82,205)',
'mediumorchid2': 'rgb(209,95,238)', 'cyan2': 'rgb(0,238,238)', 'cyan3': 'rgb(0,205,205)', 'gray23':
'rgb(59,59,59)', 'cyan1': 'rgb(0,255,255)', 'gray25': 'rgb(64,64,64)', 'gray24': 'rgb(61,61,61)',
'cyan4': 'rgb(0,139,139)', 'darkviolet': 'rgb(148,0,211)', 'gray29': 'rgb(74,74,74)', 'grey83':
'rgb(212,212,212)', 'darkgray': 'rgb(169,169,169)', 'darkmagenta': 'rgb(139,0,139)', 'navy':
'rgb(0,0,128)', 'ghostwhite': 'rgb(248,248,255)', 'darkkhaki': 'rgb(189,183,107)', 'gray94':
'rgb(240,240,240)', 'gray95': 'rgb(242,242,242)', 'gray96': 'rgb(245,245,245)', 'gray97':
'rgb(247,247,247)', 'gray90': 'rgb(229,229,229)', 'gray91': 'rgb(232,232,232)', 'gray92':
'rgb(235,235,235)', 'gray93': 'rgb(237,237,237)', 'violetred': 'rgb(208,32,144)', 'gray98':
'rgb(250,250,250)', 'cornsilk': 'rgb(255,248,220)', 'red': 'rgb(255,0,0)', 'peachpuff':
'rgb(255,218,185)', 'lightslateblue': 'rgb(132,112,255)', 'steelblue': 'rgb(70,130,180)',
'lightskyblue1': 'rgb(176,226,255)', 'lightskyblue2': 'rgb(164,211,238)', 'lightskyblue3':
'rgb(141,182,205)', 'lightslategrey': 'rgb(119,136,153)', 'blue3': 'rgb(0,0,205)', 'blue2':
'rgb(0,0,238)', 'blue4': 'rgb(0,0,139)', 'lightgrey': 'rgb(211,211,211)', 'burlywood':
'rgb(222,184,135)', 'darksalmon': 'rgb(233,150,122)', 'violetred1': 'rgb(255,62,150)', 'slategray2':
'rgb(185,211,238)', 'slategray3': 'rgb(159,182,205)', 'violetred4': 'rgb(139,34,82)', 'grey27':
'rgb(69,69,69)', 'tomato4': 'rgb(139,54,38)', 'tomato1': 'rgb(255,99,71)', 'tomato3':
'rgb(205,79,57)', 'tomato2': 'rgb(238,92,66)', 'grey21': 'rgb(54,54,54)', 'grey75':
'rgb(191,191,191)', 'mistyrose': 'rgb(255,228,225)', 'grey23': 'rgb(59,59,59)', 'orangered':
'rgb(255,69,0)', 'navajowhite': 'rgb(255,222,173)', 'grey78': 'rgb(199,199,199)', 'slategray':
'rgb(112,128,144)', 'grey79': 'rgb(201,201,201)', 'lightcyan': 'rgb(224,255,255)', 'mistyrose1':
'rgb(255,228,225)', 'white': 'rgb(255,255,255)', 'gray26': 'rgb(66,66,66)', 'tomato':
'rgb(255,99,71)', 'gray46': 'rgb(117,117,117)', 'gray44': 'rgb(112,112,112)', 'gray43':
'rgb(110,110,110)', 'seagreen4': 'rgb(46,139,87)', 'seagreen3': 'rgb(67,205,128)', 'seagreen2':
'rgb(78,238,148)', 'seagreen1': 'rgb(84,255,159)', 'paleturquoise2': 'rgb(174,238,238)',
'mistyrose4': 'rgb(139,125,123)', 'paleturquoise1': 'rgb(187,255,255)', 'limegreen':
'rgb(50,205,50)', 'lightyellow': 'rgb(255,255,224)', 'mistyrose3': 'rgb(205,183,181)', 'palegreen1':
'rgb(154,255,154)', 'palegreen3': 'rgb(124,205,124)', 'palegreen2': 'rgb(144,238,144)',
'palegreen4': 'rgb(84,139,84)', 'mistyrose2': 'rgb(238,213,210)', 'grey43': 'rgb(110,110,110)',
'turquoise3': 'rgb(0,197,205)', 'turquoise2': 'rgb(0,229,238)', 'lightcyan3': 'rgb(180,205,205)',
'lightcyan2': 'rgb(209,238,238)', 'lightcyan4': 'rgb(122,139,139)', 'rosybrown': 'rgb(188,143,143)',
'turquoise4': 'rgb(0,134,139)', 'whitesmoke': 'rgb(245,245,245)', 'lightblue': 'rgb(173,216,230)',
'snow': 'rgb(255,250,250)', 'grey46': 'rgb(117,117,117)', 'gray58': 'rgb(148,148,148)', 'gray59':
'rgb(150,150,150)', 'grey45': 'rgb(115,115,115)', 'olivedrab4': 'rgb(105,139,34)', 'purple4':
'rgb(85,26,139)', 'gray52': 'rgb(133,133,133)', 'gray53': 'rgb(135,135,135)', 'purple1':
'rgb(155,48,255)', 'olivedrab1': 'rgb(192,255,62)', 'olivedrab2': 'rgb(179,238,58)', 'olivedrab3':
'rgb(154,205,50)', 'orangered3': 'rgb(205,55,0)', 'orangered2': 'rgb(238,64,0)', 'orangered1':
'rgb(255,69,0)', 'orangered4': 'rgb(139,37,0)', 'thistle3': 'rgb(205,181,205)', 'thistle2':
'rgb(238,210,238)', 'thistle1': 'rgb(255,225,255)', 'salmon': 'rgb(250,128,114)', 'thistle4':
'rgb(139,123,139)', 'oldlace': 'rgb(253,245,230)', 'gray19': 'rgb(48,48,48)', 'darkseagreen4':
'rgb(105,139,105)', 'darkseagreen3': 'rgb(155,205,155)', 'darkseagreen2': 'rgb(180,238,180)',
'darkseagreen1': 'rgb(193,255,193)', 'gray14': 'rgb(36,36,36)', 'lightpink1': 'rgb(255,174,185)',
'grey100': 'rgb(255,255,255)', 'gray17': 'rgb(43,43,43)', 'gold': 'rgb(255,215,0)', 'slategray1':
'rgb(198,226,255)', 'burlywood3': 'rgb(205,170,125)', 'lightsalmon4': 'rgb(139,87,66)',
'lightsalmon2': 'rgb(238,149,114)', 'lightsalmon3': 'rgb(205,129,98)', 'mediumforestgreen':
'rgb(50,129,75)', 'lightsalmon1': 'rgb(255,160,122)', 'green4': 'rgb(0,139,0)', 'green1':
'rgb(0,255,0)', 'green3': 'rgb(0,205,0)', 'green2': 'rgb(0,238,0)', 'darkslategray1':
'rgb(151,255,255)', 'darkslategray2': 'rgb(141,238,238)', 'darkslategray3': 'rgb(121,205,205)',
'darkslategray4': 'rgb(82,139,139)', 'steelblue1': 'rgb(99,184,255)', 'papayawhip':
'rgb(255,239,213)', 'black': 'rgb(0,0,0)', 'orchid4': 'rgb(139,71,137)', 'orchid1':
'rgb(255,131,250)', 'orchid2': 'rgb(238,122,233)', 'orchid3': 'rgb(205,105,201)', 'antiquewhite3':
'rgb(205,192,176)', 'antiquewhite2': 'rgb(238,223,204)', 'antiquewhite1': 'rgb(255,239,219)',
'hotpink': 'rgb(255,105,180)', 'antiquewhite4': 'rgb(139,131,120)', 'bisque4': 'rgb(139,125,107)',
'bisque1': 'rgb(255,228,196)', 'bisque2': 'rgb(238,213,183)', 'bisque3': 'rgb(205,183,158)', 'gray':
'rgb(126,126,126)', 'darkturquoise': 'rgb(0,206,209)', 'slategray4': 'rgb(108,123,139)',
'cornsilk1': 'rgb(255,248,220)', 'gray22': 'rgb(56,56,56)', 'plum4': 'rgb(139,102,139)', 'plum3':
'rgb(205,150,205)', 'plum2': 'rgb(238,174,238)', 'plum1': 'rgb(255,187,255)', 'lightblue3':
'rgb(154,192,205)', 'grey49': 'rgb(125,125,125)', 'grey48': 'rgb(122,122,122)', 'thistle':
'rgb(216,191,216)', 'violet': 'rgb(238,130,238)', 'darkorchid4': 'rgb(104,34,139)', 'grey42':
'rgb(107,107,107)', 'grey41': 'rgb(105,105,105)', 'grey40': 'rgb(102,102,102)', 'grey47':
'rgb(120,120,120)', 'darkorchid1': 'rgb(191,62,255)', 'darkorchid2': 'rgb(178,58,238)',
'darkorchid3': 'rgb(154,50,205)', 'honeydew': 'rgb(240,255,240)', 'gray18': 'rgb(46,46,46)',
'cornflowerblue': 'rgb(100,149,237)', 'darkblue': 'rgb(0,0,139)', 'gray15': 'rgb(38,38,38)',
'gray16': 'rgb(41,41,41)', 'gray82': 'rgb(209,209,209)', 'gray10': 'rgb(26,26,26)', 'gray11':
'rgb(28,28,28)', 'gray12': 'rgb(31,31,31)', 'gray13': 'rgb(33,33,33)', 'palevioletred4':
'rgb(139,71,93)', 'palevioletred1': 'rgb(255,130,171)', 'palevioletred2': 'rgb(238,121,159)',
'palevioletred3': 'rgb(205,104,137)', 'mediumpurple': 'rgb(147,112,219)', 'lightblue2':
'rgb(178,223,238)', 'darkcyan': 'rgb(0,139,139)', 'lightblue1': 'rgb(191,239,255)', 'lightblue4':
'rgb(104,131,139)', 'darkred': 'rgb(139,0,0)', 'lavenderblush1': 'rgb(255,240,245)',
'lavenderblush3': 'rgb(205,193,197)', 'lavenderblush2': 'rgb(238,224,229)', 'lavenderblush4':
'rgb(139,131,134)', 'grey99': 'rgb(252,252,252)', 'mediumturquoise': 'rgb(72,209,204)', 'grey38':
'rgb(97,97,97)', 'grey39': 'rgb(99,99,99)', 'grey36': 'rgb(92,92,92)', 'grey37': 'rgb(94,94,94)',
'grey34': 'rgb(87,87,87)', 'grey35': 'rgb(89,89,89)', 'aqua': 'rgb(0,255,255)', 'grey33':
'rgb(84,84,84)', 'grey30': 'rgb(77,77,77)', 'grey31': 'rgb(79,79,79)', 'gray83': 'rgb(212,212,212)',
'sienna4': 'rgb(139,71,38)', 'gray81': 'rgb(207,207,207)', 'gray80': 'rgb(204,204,204)', 'sienna1':
'rgb(255,130,71)', 'gray86': 'rgb(219,219,219)', 'sienna3': 'rgb(205,104,57)', 'sienna2':
'rgb(238,121,66)', 'lightyellow1': 'rgb(255,255,224)', 'lightyellow2': 'rgb(238,238,209)',
'lightyellow3': 'rgb(205,205,180)', 'lightyellow4': 'rgb(139,139,122)', 'lightgoldenrod':
'rgb(238,221,130)', 'gray87': 'rgb(222,222,222)', 'navajowhite4': 'rgb(139,121,94)', 'mediumorchid':
'rgb(186,85,211)', 'olivedrab': 'rgb(107,142,35)', 'navajowhite1': 'rgb(255,222,173)',
'navajowhite2': 'rgb(238,207,161)', 'darkgoldenrod1': 'rgb(255,185,15)', 'darkgoldenrod2':
'rgb(238,173,14)', 'palegreen': 'rgb(152,251,152)', 'seashell': 'rgb(255,245,238)', 'aquamarine':
'rgb(127,255,212)', 'tan4': 'rgb(139,90,43)', 'tan3': 'rgb(205,133,63)', 'tan2': 'rgb(238,154,73)',
'tan1': 'rgb(255,165,79)', 'lawngreen': 'rgb(124,252,0)', 'maroon': 'rgb(176,48,96)', 'darkorange4':
'rgb(139,69,0)', 'mintcream': 'rgb(245,255,250)', 'darkorange1': 'rgb(255,127,0)', 'darkorange3':
'rgb(205,102,0)', 'darkorange2': 'rgb(238,118,0)', 'saddlebrown': 'rgb(139,69,19)', 'goldenrod4':
'rgb(139,105,20)', 'goldenrod1': 'rgb(255,193,37)', 'mediumgoldenrod': 'rgb(209,193,102)',
'goldenrod3': 'rgb(205,155,29)', 'grey32': 'rgb(82,82,82)', 'darkgoldenrod': 'rgb(184,134,11)',
'lightcyan1': 'rgb(224,255,255)', 'sandybrown': 'rgb(244,164,96)', 'turquoise1': 'rgb(0,245,255)',
'ivory3': 'rgb(205,205,193)', 'ivory2': 'rgb(238,238,224)', 'ivory1': 'rgb(255,255,240)',
'burlywood2': 'rgb(238,197,145)', 'mediumseagreen': 'rgb(60,179,113)', 'ivory4': 'rgb(139,139,131)',
'darkorange': 'rgb(255,140,0)', 'grey9': 'rgb(23,23,23)', 'firebrick1': 'rgb(255,48,48)',
'burlywood4': 'rgb(139,115,85)', 'gray89': 'rgb(227,227,227)', 'palegoldenrod': 'rgb(238,232,170)',
'gray85': 'rgb(217,217,217)', 'violetred3': 'rgb(205,50,120)', 'gray84': 'rgb(214,214,214)',
'grey72': 'rgb(184,184,184)', 'grey73': 'rgb(186,186,186)', 'grey70': 'rgb(179,179,179)', 'grey71':
'rgb(181,181,181)', 'grey76': 'rgb(194,194,194)', 'grey77': 'rgb(196,196,196)', 'grey74':
'rgb(189,189,189)', 'gray88': 'rgb(224,224,224)', 'pink1': 'rgb(255,181,197)', 'pink3':
'rgb(205,145,158)', 'pink2': 'rgb(238,169,184)', 'pink4': 'rgb(139,99,108)', 'gray47':
'rgb(120,120,120)', 'darkolivegreen4': 'rgb(110,139,61)', 'gray45': 'rgb(115,115,115)',
'lightsteelblue': 'rgb(176,196,222)', 'darkolivegreen1': 'rgb(202,255,112)', 'gray42':
'rgb(107,107,107)', 'darkolivegreen3': 'rgb(162,205,90)', 'darkolivegreen2': 'rgb(188,238,104)',
'lavenderblush': 'rgb(255,240,245)', 'gray49': 'rgb(125,125,125)', 'gray48': 'rgb(122,122,122)',
'azure1': 'rgb(240,255,255)', 'azure3': 'rgb(193,205,205)', 'azure2': 'rgb(224,238,238)', 'indigo':
'rgb(75,0,130)', 'azure4': 'rgb(131,139,139)', 'firebrick': 'rgb(178,34,34)', 'indianred':
'rgb(205,92,92)', 'darkolivegreen': 'rgb(85,107,47)', 'grey55': 'rgb(140,140,140)',
'lightsteelblue1': 'rgb(202,225,255)', 'lightsteelblue2': 'rgb(188,210,238)', 'lightsteelblue3':
'rgb(162,181,205)', 'lightsteelblue4': 'rgb(110,123,139)', 'darkgoldenrod4': 'rgb(139,101,8)',
'grey50': 'rgb(127,127,127)', 'magenta4': 'rgb(139,0,139)', 'magenta3': 'rgb(205,0,205)',
'magenta2': 'rgb(238,0,238)', 'magenta1': 'rgb(255,0,255)', 'darkgoldenrod3': 'rgb(205,149,12)',
'chartreuse': 'rgb(127,255,0)', 'mediumslateblue': 'rgb(123,104,238)', 'grey52': 'rgb(133,133,133)',
'springgreen': 'rgb(0,255,127)', 'grey53': 'rgb(135,135,135)', 'lightsalmon': 'rgb(255,160,122)',
'gray21': 'rgb(54,54,54)', 'turquoise': 'rgb(64,224,208)', 'grey8': 'rgb(20,20,20)', 'gray20':
'rgb(51,51,51)', 'grey6': 'rgb(15,15,15)', 'grey7': 'rgb(18,18,18)', 'grey4': 'rgb(10,10,10)',
'grey5': 'rgb(13,13,13)', 'grey2': 'rgb(5,5,5)', 'grey3': 'rgb(8,8,8)', 'grey0': 'rgb(0,0,0)',
'grey1': 'rgb(3,3,3)', 'gray50': 'rgb(127,127,127)', 'goldenrod': 'rgb(218,165,32)', 'gray51':
'rgb(130,130,130)', 'navajowhite3': 'rgb(205,179,139)', 'darkgreen': 'rgb(0,100,0)', 'gray27':
'rgb(69,69,69)', 'peachpuff4': 'rgb(139,119,101)', 'gray54': 'rgb(138,138,138)', 'slateblue4':
'rgb(71,60,139)', 'slateblue3': 'rgb(105,89,205)', 'slateblue2': 'rgb(122,103,238)', 'slateblue1':
'rgb(131,111,255)', 'peachpuff3': 'rgb(205,175,149)', 'purple3': 'rgb(125,38,205)', 'gray28':
'rgb(71,71,71)', 'purple2': 'rgb(145,44,238)', 'peachpuff1': 'rgb(255,218,185)', 'royalblue4':
'rgb(39,64,139)', 'yellowgreen': 'rgb(154,205,50)', 'royalblue1': 'rgb(72,118,255)', 'peachpuff2':
'rgb(238,203,173)', 'lightgoldenrod3': 'rgb(205,190,112)', 'lightgoldenrod2': 'rgb(238,220,130)',
'orchid': 'rgb(218,112,214)', 'purple': 'rgb(160,32,240)', 'deeppink4': 'rgb(139,10,80)', 'wheat4':
'rgb(139,126,102)', 'wheat1': 'rgb(255,231,186)', 'wheat3': 'rgb(205,186,150)', 'wheat2':
'rgb(238,216,174)', 'indianred4': 'rgb(139,58,58)', 'coral2': 'rgb(238,106,80)', 'coral1':
'rgb(255,114,86)', 'rosybrown4': 'rgb(139,105,105)', 'deepskyblue3': 'rgb(0,154,205)', 'rosybrown2':
'rgb(238,180,180)', 'rosybrown1': 'rgb(255,193,193)', 'indianred3': 'rgb(205,85,85)', 'khaki':
'rgb(240,230,140)', 'wheat': 'rgb(245,222,179)', 'gray55': 'rgb(140,140,140)', 'deepskyblue':
'rgb(0,191,255)', 'grey44': 'rgb(112,112,112)', 'gray56': 'rgb(143,143,143)', 'plum':
'rgb(221,160,221)', 'beige': 'rgb(245,245,220)', 'azure': 'rgb(240,255,255)', 'honeydew1':
'rgb(240,255,240)', 'dodgerblue2': 'rgb(28,134,238)', 'dodgerblue3': 'rgb(24,116,205)',
'dodgerblue4': 'rgb(16,78,139)', 'gray57': 'rgb(145,145,145)', 'mediumvioletred': 'rgb(199,21,133)',
'snow4': 'rgb(139,137,137)', 'snow2': 'rgb(238,233,233)', 'snow3': 'rgb(205,201,201)', 'snow1':
'rgb(255,250,250)', 'lightcoral': 'rgb(240,128,128)', 'grey29': 'rgb(74,74,74)', 'grey28':
'rgb(71,71,71)', 'grey25': 'rgb(64,64,64)', 'grey24': 'rgb(61,61,61)', 'mediumspringgreen':
'rgb(0,250,154)', 'grey26': 'rgb(66,66,66)', 'fuchsia': 'rgb(255,0,255)', 'grey20': 'rgb(51,51,51)',
'blanchedalmond': 'rgb(255,235,205)', 'grey22': 'rgb(56,56,56)', 'gray78': 'rgb(199,199,199)',
'gray79': 'rgb(201,201,201)', 'gray76': 'rgb(194,194,194)', 'gray77': 'rgb(196,196,196)', 'gray74':
'rgb(189,189,189)', 'gray75': 'rgb(191,191,191)', 'gray72': 'rgb(184,184,184)', 'gray73':
'rgb(186,186,186)', 'gray70': 'rgb(179,179,179)', 'gray71': 'rgb(181,181,181)', 'ivory':
'rgb(255,255,240)', 'lightpink2': 'rgb(238,162,173)', 'lightpink3': 'rgb(205,140,149)',
'forestgreen': 'rgb(34,139,34)', 'lightpink4': 'rgb(139,95,101)', 'cornsilk4': 'rgb(139,136,120)',
'cornsilk2': 'rgb(238,232,205)', 'cornsilk3': 'rgb(205,200,177)', 'lightgoldenrod4':
'rgb(139,129,76)', 'grey90': 'rgb(229,229,229)', 'grey91': 'rgb(232,232,232)', 'grey92':
'rgb(235,235,235)', 'grey93': 'rgb(237,237,237)', 'grey94': 'rgb(240,240,240)', 'grey95':
'rgb(242,242,242)', 'grey96': 'rgb(245,245,245)', 'grey97': 'rgb(247,247,247)', 'grey98':
'rgb(250,250,250)', 'lightgoldenrod1': 'rgb(255,236,139)', 'royalblue3': 'rgb(58,95,205)',
'cadetblue': 'rgb(95,158,160)', 'royalblue2': 'rgb(67,110,238)', 'slateblue': 'rgb(106,90,205)',
'honeydew3': 'rgb(193,205,193)', 'dimgray': 'rgb(105,105,105)', 'seagreen': 'rgb(46,139,87)',
'cadetblue4': 'rgb(83,134,139)', 'cadetblue3': 'rgb(122,197,205)', 'cadetblue2': 'rgb(142,229,238)',
'cadetblue1': 'rgb(152,245,255)', 'paleturquoise': 'rgb(175,238,238)', 'deeppink3':
'rgb(205,16,118)', 'deeppink2': 'rgb(238,18,137)', 'deeppink1': 'rgb(255,20,147)', 'darkorchid':
'rgb(153,50,204)', 'hotpink3': 'rgb(205,96,144)', 'hotpink2': 'rgb(238,106,167)', 'hotpink1':
'rgb(255,110,180)', 'chocolate': 'rgb(210,105,30)', 'hotpink4': 'rgb(139,58,98)', 'honeydew2':
'rgb(224,238,224)', 'navyblue': 'rgb(0,0,128)', 'mediumaquamarine': 'rgb(102,205,170)'}
...
def numsortkey(s):
    ## returns a list of integers that can be derived from a string
    if type(s)!=str:
        return s
    r1=[]
    cn=''
...
def vcolor(sc0,**cod1):
    ...
    if 'index1' in cod1 and cvc:
        ...
        cvc1.linearcolorimax=max(float(cod1['index1']),cvc1.linearcolorimax)
        cvc1.linearcolorimin=min(float(cod1['index1']),cvc1.linearcolorimin)
    ...
    if sc2=='NAN':
        ...
        if ',' in sc1:
            try:
                s1 = ast.literal_eval(sc1)  ## 3-6-22 was 'eval' but have changed this due to potential security issue
                if len(s1)==3:
...
def vcleancomment(t):
    return t.replace('<!--','<Start_HTML_Comment>').replace('-->','<End_HTML_Comment>')
...
# 5: listg2[1:-1]: list of minor demarkations (labels in string format),
# 6: listc2[1:-1]]: list of minor demarkations in string format
    ...
    if "precise" in dgrid1:
        gprecise1=bool(dgrid1['precise'])
    if "inclusive" in dgrid1:
        ginclusive1=bool(dgrid1['inclusive'])
    else:
        ginclusive1=False
    ...
    if ginclusive1:
        q1 = int(round(z1))
        q2 = int(round(z2))
        listg1 = list(range(q1,q2+1))
        listc1 = []
        cdel1 = max(1.0,float(z2-z1))
        for q0 in listg1:
            listc1.append((q0-z1)*(z2c-z1c)/cdel1+z1c)
        return [(float(q1),float(q2)),1.0,listg1,listc1,1.0,[],[]]
...
vvsvge1 = {"&": "&amp;",'"': "&quot;","'": "&apos;",">": "&gt;","<": "&lt;"}
def vvsvge(text):
    return "".join(vvsvge1.get(c,c) for c in text)
vvhtmle1 = {"&": "\\\&",'"': "\\\"","'": "\\\'",">": "\\\>","<": "\\\<"}
def vvhtmle(text):
    return "".join(vvhtmle1.get(c,c) for c in text)
vvmvge1 = {'"': "'"}
def vvmvge(text):
    return "".join(vvmvge1.get(c,c) for c in text)
...
class VSGcanvas(list):
    ...
    def __init__(self,**d3):
        ...
        self.bg = 'white' ##hexcolor('gray90') ## alternative hexcolor('gray60') sets background color
        ...
        self.svgstrokezoom=False ## Setting this to True creates strokewidths that stay constant with zoom in SVG
                                 ## Setting to a fixed value uses that value in vw units for the stroke
...
## need to open a tk window here to get proper text string sizes
    ...
    def transform(self,**dt1):
        ...
        for v1 in il1:
            if v1 in dir(transformedcopy):
                if (v1[0]=='x' or v1[-1]=='x') and not(v1=='ymax'):
                    del1=xdel1
                    sc1=xsc1
                else:
                    del1=ydel1
    ...
    def vinfo(self,*dz0,**dz2):
        ...
        if sys.argv and sys.argv[0]!='-c':
            try:
                MyDir1 = os.path.dirname(sys.argv[0])
                if not(MyDir1):
                    MyDir1 = os.getcwd()
                vinfo1+=delimiter+'  Script='+os.path.basename(sys.argv[0])+' | Ver='+modification_date(sys.argv[0])+' | Dir='+MyDir1+' '
            except:
                pass
        if dz2['lines']==2: return vinfo1
        allvar=set(list(globals().keys())+list(locals().keys()))
        if1=0
        for vari1 in allvar:
            if vari1 in globals():
                if hasattr(globals()[vari1],'seek'):
                    if1+=1
                    NN1=globals()[vari1]
                    vinfo1+=delimiter+'  Opened_File_'+str(if1)+': '+vari1+"='"+NN1.name+"' | Mode='"+NN1.mode +"' | ModStamp="+modification_date(NN1.name)+' '+delimiter
            elif vari1 in locals():
                if hasattr(locals()[vari1],'seek'):
                    if1+=1
                    NN1=locals()[vari1]
                    vinfo1+=delimiter+'  Opened_File_'+str(if1)+': '+vari1+"='"+NN1.name+"' | Mode='"+NN1.mode +"' | ModStamp="+modification_date(NN1.name)+' '+delimiter
        return vinfo1[:-len(delimiter)]
	...
	be passed to this is "scale", which is the scale of the eventual drawing (recommended as ~3.0 for line drawings and 1.0 for vector)'''
        ...
        self.sc = self.scale
        if 'scale' in dw1:
            self.sc=float(dw1['scale'])
        if 'Scale' in dw1:
            self.sc=float(dw1['Scale'])
        if self.sc in ("auto","Auto"):
            self.sc=1.0
        self.cvwidth=self.sc*(self.xmax-self.xmin+self.marginl+self.marginr)
        self.cvheight=self.sc*(self.ymax-self.ymin+self.margint+self.marginb)
        self.cvxd=self.marginl-self.xmin
        self.cvyd=self.marginb-self.ymin
        for s in self:
            ## canvas x and y values
            s.cvx1=(s.x1+self.cvxd)*self.sc
            s.cvx2=(s.x2+self.cvxd)*self.sc
            s.cvwidth=s.cvx2-s.cvx1
            s.cvy1=self.cvheight-(s.y1+self.cvyd)*self.sc
            s.cvy2=self.cvheight-(s.y2+self.cvyd)*self.sc
            s.cvheight=s.cvy2-s.cvy1
            s.cvxc=(s.xc+self.cvxd)*self.sc
            s.cvyc=self.cvheight-(s.yc+self.cvyd)*self.sc
            ...
            s.cvxcr=(s.xcr+self.cvxd)*self.sc
            s.cvycr=self.cvheight-(s.ycr+self.cvyd)*self.sc
            ...
            if s.obj=='text':
                if type(self.fontscale)==str:
                    if type(self.fontscale)==str and self.fontscale.lower()=='auto':
                        s.cvfs=int(float(s.font['size'])*self.sc)
                ...
                if 'nonrotwidth' in dir(s) and s.nonrotwidth>0:
                    ...
                    s.cvnonrotwidth=s.nonrotwidth*self.sc
            ...
            if type(self.strokescale)==str:
                if type(self.strokescale)==str and self.strokescale.lower()=='auto':
                    s.cvstrokewidth=float(s.strokewidth)*self.sc
            ...
            if s.obj in ('polygon','polyline'):
                ...
                for (xa1,ya1) in s.points:
                    s.cvpoints.append([(xa1+self.cvxd)*self.sc,self.cvheight-(ya1+self.cvyd)*self.sc])
                s.cvipoints=[]
                for (xa1,ya1) in s.points:
                    s.cvipoints.append((int((xa1+self.cvxd)*self.sc),int(self.cvheight-(ya1+self.cvyd)*self.sc)))
            if s.obj =='polyline':
                s.tkpoints=[]
                s.fill=none
                for (xa1,ya1) in s.points:
                    s.tkpoints.append([(xa1+self.cvxd)*self.sc,self.cvheight-(ya1+self.cvyd)*self.sc])
    ...
    def TKwrite(self,filepath=DefaultFileBase+'.ps',display=True):
        ...
        def tkweblink(event):
            ...
            if type(sitedict1[(tkcanvas,tkevent)])=='function':
                ...
                try:
                    webbrowser.open(sitedict1[(tkcanvas,tkevent)])
                except:
                    pass
    ...
    def MVGwrite(self,filepath=DefaultFileBase+'.tif',display=True,tempfilepath="temp.mvg"):
...
##                    if s.vjust=='bottom': GMcvgcycorr-=s.cvxr
                ...
                if s.rotate!=0:
                    ...
                    F1.write('translate '+mstr(-s.cvgcx)+','+mstr(-GMcvgcycorr)+ ' \n')
                    F1.write('text '+mstr(s.cvgcx)+','+mstr(GMcvgcycorr)+' "'+vvmvge(s.text)+'"'+' \n')
                else:
                    F1.write('gravity '+gravitydict[(s.hjust,'center')]+'\n')
                    F1.write('text '+mstr(s.cvgcx)+','+mstr(GMcvgcycorr)+' "'+vvmvge(s.text)+'"'+' \n')
        ...
        try:
            ...
            try:
                print('GraphicsMagick convert attempt failed, trying ImageMagick')
                check_output(["convert",tempfilepath,filepathfull]) ## then try imagemagick
            except:
                print('Unable to convert MVG File with ImageMagick, trying PIL conversion')
                if Imageimported1:
                    try:
    ...
    def SVGwrite(self,filepath=DefaultFileBase+'.svg',display=True):
        ...
        svgheader = '<!--\n'+vvsvge(vcleancomment(vSysLogInfo1))+'\n-->\n'
        ...
        for s in self:
            if s.obj=='text':
                ...
                if s.cvstrokewidth>0:
                    ...
                    if s.svgstrokezoom==False:
                        outline1+=' stroke-width='+svnum(s.cvstrokewidth)
                    elif s.svgstrokezoom==True:
                        outline1+=' stroke-width='+svnum(s.cvstrokewidth*100/int(self.cvwidth+0.001))[:-1]+'vw"'
                    else:
                        outline1+=' stroke-width='+svnum(s.svgstrokezoom*100/int(self.cvwidth+0.001))[:-1]+'vw"'
            ...
            if s.popup!='':
                ...
                outline1='<a onclick="javascript:window.alert('+"'"+vvsvge(s.popup)+"');return false"+'" >'+outline1+'></a'
            elif s.detail!='':
                ## outline1=outline1.rstrip('/')
                outline1='<a onclick="javascript:window.alert('+"'"+vvsvge(s.detail)+"');return false"+'" >'+outline1+'></a'
            if s.xlink!='':
                outline1='<a xlink:href="'+vvsvge(s.xlink)+'" >'+outline1+'></a'
            if s.mouseover!='':
                outline1='<a xlink:href="http://#'+vvsvge(s.mouseover)+'" >'+outline1+'></a'
        ...
        if display:
            ...
            if not(displaysuccess):
                try:
                    webbrowser.open('file://'+filepathfull)
                    print("Wrote '"+os.path.basename(filepathfull)+"': browser should start in a few seconds")
                except:
    ...
    def HTML5write(self,filepath=DefaultFileBase+'.html',display=True):
        ''' filepath=<path.svg>, display=True/False
            Render the image as a SVG web page instance with active links
            Then
               1.  Display the canvas (if "display=True" in call)
               2.  Write the image to an svg file ( "filepath=<filepath> in call)'''
        htmlHeader1='''<!doctype html>\r<html>\r<head>\r<meta charset="utf-8" />\r<title>VSG Canvas Viewer- HTML5</title>\r'''
        htmlHeader2='<!--\n\n'+vvhtmle(vcleancomment(vSysLogInfo1))+'\n-->\n'
        htmlHeader3='''<script type="text/javascript">\r<!--\rfunction draw() {\rvar canvas = document.getElementById('cav1');\rif (canvas.getContext) {\rvar ctx = canvas.getContext('2d');\r'''
        if self.scale in ('auto','Auto'):
            if abs(self.xmax-self.xmin)>self.screenwidth:
            ...
            self.windowcalc()
        file_output1=open(filepath,'w')
        file_output1.write(htmlHeader1)
        file_output1.write(htmlHeader2)
        file_output1.write(htmlHeader3)
        ...
        for s in self:
            ...
            if s.obj=='text':
                ...
                outline1+='ctx.fillText(%s,%s,%s);\r'%('"'+vvhtmle(s.text).replace('\r',' ').replace('\n',' ')+'"',svnum(s.cvxcr),svnum(s.cvyt))
...
##                    print('Unsuccessful at inline display of image '+filepath+', using external display')
    ...
    def PILwrite(self,filepath=DefaultFileBase+'.png',display=True):
        ...
        pilimage=Image.new('RGB',(int(self.cvwidth),int(self.cvheight)),self.bg)
        draw1=ImageDraw.Draw(pilimage)
        for s in self:
            if s.obj=='line':
                if s.cvstroke:
                    draw1.line([int(s.cvx1),int(s.cvy1),int(s.cvx2),int(s.cvy2)],s.cvstroke,s.cvistrokewidth)
            if s.obj in ('square','rect','rectangle'):
                if s.fill==none and s.cvstroke:
                    draw1.line([int(s.cvx1),int(s.cvy1),int(s.cvx2),int(s.cvy1)],s.cvstroke,s.cvistrokewidth)
                    draw1.line([int(s.cvx1),int(s.cvy1),int(s.cvx1),int(s.cvy2)],s.cvstroke,s.cvistrokewidth)
                    draw1.line([int(s.cvx1),int(s.cvy2),int(s.cvx2),int(s.cvy2)],s.cvstroke,s.cvistrokewidth)
                    draw1.line([int(s.cvx2),int(s.cvy1),int(s.cvx2),int(s.cvy2)],s.cvstroke,s.cvistrokewidth)                    
                elif s.strokewidth>0:
                    if s.cvstroke:
                        draw1.rectangle([int(s.cvmx1),int(s.cvmy1),int(s.cvmx2),int(s.cvmy2)],s.cvstroke)
                    if s.cvfill:
                        draw1.rectangle([int(s.cvmx1+s.cvstrokewidth),int(s.cvmy1+s.cvstrokewidth),int(s.cvmx2-s.cvstrokewidth),int(s.cvmy2-s.cvstrokewidth)],s.cvfill)
                else:
                    if s.cvfill:
                        draw1.rectangle([int(s.cvmx1),int(s.cvmy1),int(s.cvmx2),int(s.cvmy2)],s.cvfill)
            if s.obj in ('circle','ellipse'):
                if s.strokewidth>0:
                    if s.cvstroke:
                        draw1.ellipse([int(s.cvmx1),int(s.cvmy1),int(s.cvmx2),int(s.cvmy2)],s.cvstroke)
                    if s.cvfill:
                        draw1.ellipse([int(s.cvmx1+s.cvstrokewidth),int(s.cvmy1+s.cvstrokewidth),int(s.cvmx2-s.cvistrokewidth),int(s.cvmy2-s.cvstrokewidth)],s.cvfill)
                else:
                    if s.cvfill:
                        draw1.ellipse([int(s.cvmx1),int(s.cvmy1),int(s.cvmx2),int(s.cvmy2)],s.cvfill)
            if s.obj in ('arc','pieslice'):
                if self.arcscale==0:
                    self.arcscale=0.1
                if s.strokewidth>0:
                    if s.cvstroke:
                        draw1.pieslice([int(s.cvmx1),int(s.cvmy1),int(s.cvmx2),int(s.cvmy2)], int((360.0*(s.a0)/self.arcscale-90)%360.0),int((360.0*(s.a0+s.ad)/self.arcscale-90)%360.0), fill=s.cvstroke)
                    if s.cvfill and s.cvstroke:
                        draw1.pieslice([int(s.cvmx1+s.cvstrokewidth),int(s.cvmy1+s.cvstrokewidth),int(s.cvmx2-s.cvstrokewidth),int(s.cvmy2-s.cvstrokewidth)],int((360.0*(s.a0)/self.arcscale-90)%360.0),int((360.0*(s.a0+s.ad)/self.arcscale-90)%360.0), fill=s.cvfill,outline=s.cvstroke)
                else:
                    if s.cvfill:
                        draw1.pieslice([int(s.cvmx1),int(s.cvmy1),int(s.cvmx2),int(s.cvmy2)],int((360.0*(s.a0)/self.arcscale-90)%360.0), int((360.0*(s.a0+s.ad)/self.arcscale-90)%360.0), fill=s.cvfill)
                    
            if s.obj in ('polyline','polygon'):
                if s.obj=='polygon' and s.cvfill:
                    draw1.polygon(s.cvipoints,s.cvfill)
                if s.strokewidth>0 and s.cvstroke:
                    draw1.line(s.cvipoints,s.cvstroke,s.cvistrokewidth)
            if s.obj=='text':
                ...
                try:
                    ts000 = font11.getbbox(s.text)
                    ts1=[font11.getlength(s.text),abs(ts000[3]-ts000[1])] 
                except:
                ...
                if s.cvfill:
                    draw1.text([int(s.cvix1),int(s.cviy1)],s.text,fill=s.cvfill,font=font11)
    ...
    def vwrite(self,filepath=DefaultFileBase,display=True,live=False,**vwriteparams):
        ''' filepath=<path.html/jpg/tif/png/svg>, display=True/False
            ...
            Then
               ...
               2.  Write the image to a tif/jpg/png file (if "psfile=<filepath> in call)
			This will fail if the image package from python is not imported (in which case, VSG will try MVG instead)'''
        if 'scale' in vwriteparams:
            self.scale=vwriteparams['scale']
        self.sort(key=lambda u:(u.priority,u.ordinal))
        filepath=filepath.strip()
        if filepath and filepath[0]=='.':
            filepath=filepath[1:]
        if filepath.lower() in ['png','svg','ps','tif','tiff','jpg','htm','html','pdf']:
            filepath=DefaultFileBase+'.'+filepath.lower()
        if not('.' in filepath):
            if viPython:
                filepath+='.svg'
            else:
                filepath+='.html'
        self.live=live
        if self.live and filepath==DefaultFileBase+'.html':
            try:
                self.TKwrite()
                return ""
            except:
                pass
        fpext1=filepath[::-1].split('.')[0][::-1].lower()
        if fpext1=='svg':
        ...
        if fpext1=='pdf':
            if SVGPDFLibs1:
                try:
                    fpsvg1 = filepath[:-4]+'.svg'
                    fpsvg1full=os.path.abspath(fpsvg1)
                    filepathfull=os.path.abspath(filepath)
                    if viPython:
                        self.SVGwrite(filepath=fpsvg1full,display=display)
                    else:
                        self.SVGwrite(filepath=fpsvg1full,display=False)
                    d1svg = svg2rlg(fpsvg1full)
                    renderPDF.drawToFile(d1svg,filepathfull)
                    if display and not(viPython):
                        displaysuccess=False
                        try:
                            check_call(["open",filepathfull])
                        except:
                            try:
                                ...
                                try:
                                    webbrowser.open('file://'+fpsvg1full)
                                    print("Wrote '"+os.path.basename(filepathfull)+"': browser should start in a few seconds")
                                except:
                                    print("File "+filepathfull+" is written but was not displayed due to a web-browser issue, try opening from the system file browser")
                    return ""
                    ...
                    pass
            print("Defaulting to raster rendering for PDF image.  Try installing svglib and reportlab for vector pdf")
            print("This may work with simply typing 'pip install svglib reportlab' in terminal and rerunning VSG")
            print("Or if this fails, you may need to copy the freetype folder to a new location [https://stackoverflow.com/questions/36400099/missing-header-file-for-reportlab-dependency]")
...
##        """adds a line to canvas"""
    ...
    def vcolorkey(self,*dz0,**xgd):
		...
		gsorted=True for sorting all symbol labels in the legend
		nsorted=Sorting that will try to extract numerical values as sort parameters
        '''
        gprecise1=False
        ginclusive1=False
        gsorted1=False
        nsorted1=False
        for u in list(xgd.keys()):
            xgd[u.lower()]=xgd[u]
        ...
        if "inclusive" in xgd:
            ginclusive1=bool(xgd['inclusive'])
        if "sorted" in xgd:
            gsorted1=bool(xgd['sorted'])
        if "gsorted" in xgd:
            gsorted1=bool(xgd['gsorted'])
        if "nsorted" in xgd:
            nsorted1=bool(xgd['nsorted'])
        ...
        cge=d['ckfont']['size']+0
        if External=='linear':
            xginfo=griddivlinear((clmin,clmax),(cgx,cgx+cgd),precise=gprecise1,inclusive=ginclusive1)
            gxlistN=xginfo[3]
            ...
            if 'spectrumtitle' in d:
                ...
                cgy-=cge   
            for i in range(255):
                self.vrect(y1=cgy,y2=cgy-keyheight,x1=cgx+float(i)*cgd/256.0,x2=cgx+float(i+1)*cgd/256.0+1,fill=colorvalue((clmax-clmin)*float(i)/256.0+clmin),strokewidth=0,stroke=none)
            cgyrect=cgy+0
            cgy-=keyheight
            for te1 in gxlist:
            ...
            cgy-=gcticklength+2
            self.vrect(x1=cgx,x2=cgx+cgd,y1=cgyrect,y2=cgyrect-keyheight,stroke="black",strokewidth=1,fill=none)
            ...
            if clmin==0:
                clmin=1
            for i in range(255):
                self.vrect(y1=cgy,y2=cgy-keyheight,x1=cgx+float(i)*cgd/256,x2=cgx+float(i+1)*cgd/256+1,fill=colorvalue((clmax/clmin)**(float(i)/256.0)*clmin),strokewidth=0,stroke=none)
            cgyrect=cgy+0
            cgy-=keyheight
            for te1 in gxlist:
                self.vline(x1=te1[0],x2=te1[0],y1=cgy,y2=cgy-gcticklength,stroke="gray",strokewidth=2)
            ...
            if self.logcolormin==0:
                self.logcolormin=1
            for i in range(255):
                self.vrect(y1=cgy,y2=cgy-keyheight,x1=cgx+float(i)*cgd/256.0,x2=cgx+float(i+1)*cgd/256.0+1,fill=1+int(((float(self.logcolormax)/float(self.logcolormin))**(float(i)/256.0))*self.logcolormin),strokewidth=0,stroke=none)
            cgyrect=cgy+0
            cgy-=keyheight
            for te1 in gxlist:
                self.vline(x1=te1[0],x2=te1[0],y1=cgy,y2=cgy-gcticklength,stroke="gray",strokewidth=2)
            ...
            for nu1,te1 in enumerate(gxlist):
                ...
                self.vtext(text=te2,xc=te1[0],y2=cgy,font=d['ckfont'],color=d['ckfontcolor'])
            cgy-=cge*1.5
        elif self.linearcolor:
            xginfo=griddivlinear((self.linearcolorimin,self.linearcolorimax),(cgx,cgx+cgd),precise=gprecise1,inclusive=ginclusive1)
            gxlistN=xginfo[3]
            gxlistT=('%G'%xl1 for xl1 in xginfo[2])
            gxlist=list(zip(gxlistN,gxlistT))
            gxlistNm=xginfo[6]
            ...
            if 'spectrumtitle' in d:
                ...
                cgy-=cge   
            for i in range(255):
                self.vrect(y1=cgy,y2=cgy-keyheight,x1=cgx+float(i)*cgd/256.0,x2=cgx+float(i+1)*cgd/256.0+1,fill=(self.linearcolormax-self.linearcolormin)*float(i)/256.0+self.linearcolormin,strokewidth=0,stroke=none)
            cgyrect=cgy+0
            cgy-=keyheight
            for te1 in gxlist:
                self.vline(x1=te1[0],x2=te1[0],y1=cgy,y2=cgy-gcticklength,stroke="gray",strokewidth=2)
        ...
        cgy-=cge*1.5
        if gsorted1:
            clist11 = sorted(self.colorlist, key=lambda x:x[0])
        if nsorted1:
            clist11 = sorted(self.colorlist, key=lambda x:numsortkey(x[0]))
        else:
            clist11 = self.colorlist            
        for (cg1,cn1) in clist11:
            if type(cn1)==str:
    ...
    def vread(self,vs1):
        ...
        while '=' in vs2:
            ...
            if vs4[0]=='"':
                ...
                if vs6=='NAN':
                    try:
                        vs6=ast.literal_eval(vs5[0])  ## 3-6-22 was 'eval' but have changed this due to potential security issue
                    except:
                        vs6=copy(vs5[0])
...
class VSGitem:
    def __init__(s,parentcanvas,*pr1,**d1):
        ...
        for d0 in list(d1.keys()):
            ...
            if d0.lower()=='svgstrokezoom':
                if type(d2[d0])==str:
                    s.svgstrokezoom= d2[d0].strip('"').strip("'")
                    if s.svgstrokezoom.lower().startswith('t'):
                        s.svgstrokezoom = True
                    elif s.svgstrokezoom.lower().startswith('f'):
                        s.svgstrokezoom = False
                    else:
                        s.svgstrokezoom=float(s.svgstrokezoom)
                else:
                    s.svgstrokezoom = d2[d0]
        ...
        if s.obj=='text':
            ...
            if s.metric.lower()=='tk' and tkFontimported1:
                try:
                    ...
                    d2['width']=max([s.tkfont.measure(myline1) for myline1 in multilinetext1])
                except:
                    s.metric='pil'
                    s.canvas.metric='pil'
            if s.metric.lower()=='pil' and Imageimported1:
                try:
                    ...
                    pilfont=OpenFontsTTF1[cvttf2]
                    try:
                        d2['width']=max([pilfont.getlength(myline1) for myline1 in multilinetext1])
                        vh000 = pilfont.getbbox(s.text)
                        d2['height']=abs(vh000[3]-vh000[1])
                    except:
                        d2['width']=max([pilfont.getsize(myline1)[0] for myline1 in multilinetext1])
                        d2['height']=pilfont.getsize(s.text)[1]
                except:
            ...
            if (s.metric.lower()=='pil' and not(Imageimported1)) or (s.metric.lower()=='lastditch'):
                ...
                d2['width']=max([len(myline1) for myline1 in multilinetext1])*s.font['size']/AveWidRatio1
        ...
        if len(s.canvas) and not(s.stet):
        ...
        if 'label' in dir(s):
            s.label=str(s.label)
            if type(s.labelfont)==str and s.labelfont.lower()=='auto':
                s.labelfont=deepcopy(s.font)
            if type(s.labelcolor)==str and s.labelcolor.lower()=='auto':
...
def VSGmontage(vl1,columns=3,PanelBuffer=0,align='UR'):
    ...
    Align is where an element of the montage will default to if it is smaller than the unit cell (UL=Upper Left, UR=Upper Right, UC=Upper Center
    ...
    if not('P' in align):
        ...
        for i11,vs1 in enumerate(vl1):
            ...
            if 'M' in align: yd1=vs1.ymax-(rowMaxHeight[row11]-(vs1.ymax-vs1.ymin))/2.0
...
def vLog(note,LogFileName=vDefaultLogFileName):
    LogFile=open(LogFileName,mode='a')
    LogFile.write(str(note)+'\t'+'; Time='+"{0:.2f}".format(time()-vt0)+' sec'+'\t'+strftime("D_%m_%d_%y_T_%H_%M_%S",localtime())+' \n')
    LogFile.close()
    print(str(note).split('#')[0].replace('\t',' ').strip(',') + '; Time='+"{0:.2f}".format(time()-vt0)+' sec')
def vFileInfo(FileID):
    if type(FileID)==str:
        Fn1=FileID
    else:
        Fn1=FileID.name
    ...
    return ','.join([Fn1,
                     '#',
                      'Path='+os.path.abspath(Fn1),
                        ...
                        'FullInfo='+str(s11)])
def vLogOpeningFile(FileID,kw):
    kwlist = ''
    for k in kw:
        kwlist+=k+'='+str(kw[k])+' '
    vLog("Opening "+vFileInfo(FileID)+'  '+kwlist)
def vLogClosingFile(FileID):
    vLog("Closed "+vFileInfo(FileID))
def vLogRunningFile(FileID):
    vLog("Running "+vFileInfo(FileID))
def vOpen(FileName,**kw):
    vLogOpeningFile(FileName,kw)
    if not('mode') in kw:
        kw['mode'] = 'rt'
    if FileName.endswith('.gz') or FileName.endswith('.gzip'):
        F0=gzip.open(FileName,**kw)
    elif FileName.endswith('.zip'):
        if kw['mode'].startswith('w'):
            TextFileName = FileName[::-1].replace('piz.','txt.',1)[::-1]
            F0 = open(TextFileName, **kw) ## we don't actually write zip files here... redirect to a standard text file 
        else:
            ZipFile1 = zipfile.ZipFile(FileName)
            FileList = []
            for zfn1 in ZipFile1.namelist():
                if zfn1[0]!='_':
                    FileList.extend(ZipFile1.open(zfn1,**kw))
            F0 = chain(FileList)
    else:
        F0=open(FileName,**kw)
    return F0
def vClose(Fileobject):
    vLogClosingFile(Fileobject)
    Fileobject.close()
def vhelp():
    for h1 in open(sys.argv[0],mode='r').readlines():
        if h1.strip().startswith('import ') and not('=' in h1.split('#')[0]): continue
        if h1.strip().startswith('from ') and not('=' in h1.split('#')[0]): continue
        if h1.strip().startswith('!/'): continue
        if h1.strip().startswith('vcommand'): break
        if not(h1.strip().startswith('#')) and (h1.strip().endswith(':')): break
        print(h1.rstrip())
            
vv_argv = []
vv_lowerarg = map(lambda x:x.lower(), sys.argv)
if 'vcommand' in vv_lowerarg:
    vv_argv = sys.argv[vv_lowerarg.index('vcommand'):]
else:
    try:
        for vv_myline in open(sys.argv[0],mode='r').readlines():
            if vv_myline.lower().strip().replace(' ','').replace('"',"'").startswith("vcommand()"):
                vv_argv = sys.argv[:]
    except:
        pass
def vcommand(): ## the function vcommand doesn't do anything except signal VSG to use a vcommand mode for automated parsing of command line variables
    pass
vCommand = vcommand
VCommand = vcommand
Vcommand = vcommand
if vv_argv:
    vv_argv = vv_argv[1:]
    vv_myvar0 = {}  ## keys are full variable names, values are str(values)
    vv_myvar1 = {}  ## keys are full variable names and variable abbreviations, values are full variable names
    vv_myvar2 = {}  ## keys lower case variable names and variable abbreviations, values are full variable names
    vv_myvar3 = {}  ## keys lower case variable names with the last character removed, values are full variable names
    for vv_myline in open(sys.argv[0],mode='r').readlines():
        vv_myline1 = vv_myline.split('#')[0].strip()
        if ('import' in vv_myline1) and ('VSG_Module' in vv_myline1): break
        if ('=' in vv_myline1) and not('==' in vv_myline1) and not(vv_myline1.endswith(':')):
            vv_vname = vv_myline1.split('=')[0].strip()
            if not(vv_vname) or vv_myline1.endswith('='): continue
            vv_myvar0[vv_vname] = vv_myline1.split('=',1)[-1].strip()
            vv_myvar1[vv_vname] = vv_vname
            vv_myvar2[vv_vname.lower()] = vv_vname  ## note that this means that two variable names can't be case-adjusted versions of each other
            if len(vv_vname)>1 : vv_myvar3[vv_vname.lower()[:-1]] = vv_vname
            if '#' in vv_myline:
                vv_myline2 = vv_myline.split('#',1)[1].replace(' ','').replace('\t','')
                while '=)' in vv_myline2 and '(' in vv_myline2:
                    vv_abbrev = vv_myline2.split('=)')[0][::-1].split('(')[0][::-1].strip()
                    vv_myline2 = vv_myline2.split('=)',1)[1]
                    if not(vv_abbrev in vv_myvar1):
                        vv_myvar1[vv_abbrev] = vv_vname
                        vv_myvar2[vv_abbrev.lower()] = vv_vname
    if len(vv_argv)==0: vhelp()
    for vv_a1 in vv_argv:
        if 'help'==vv_a1.lower() or vv_a1.lower()=='-h' or vv_a1.lower()=='h':
            vhelp()
            exit()
        vv_VarMnemonic = vv_a1.split('=')[0].strip()
        if vv_VarMnemonic in vv_myvar1:
            vv_VarName = vv_myvar1[vv_VarMnemonic]
        elif vv_VarMnemonic.lower() in vv_myvar2:
            vv_VarName = vv_myvar2[vv_VarMnemonic.lower()]
        elif vv_VarMnemonic.lower() in vv_myvar3:
            vv_VarName = vv_myvar3[vv_VarMnemonic.lower()]
        else:
            vLog("Unknown Variable Name in Commande Line Item: "+vv_a1)
            exit()
        vv_NewVarValue = vv_a1.split('=',1)[-1].strip().strip('"').strip("'")
        if vv_NewVarValue.startswith('['):
            vv_NewVarValue = [vv_NewVarValue11 for vv_NewVarValue11 in vv_NewVarValue.strip('[').strip(']').split(',')]
        elif vv_NewVarValue.startswith('('):
            vv_NewVarValue = (vv_NewVarValue11 for vv_NewVarValue11 in vv_NewVarValue.strip('(').strip(')').split(',')) 
        elif ',' in vv_NewVarValue:
            vv_NewVarValue = [vv_NewVarValue11 for vv_NewVarValue11 in vv_NewVarValue.split(',')] 
        else:
            try:
                vv_NewVarValue = ast.literal_eval(vv_NewVarValue)
            except:
                pass
        globals()[vv_VarName] = vv_NewVarValue
        vv_myvar0[vv_VarName] = str(vv_NewVarValue)
        print("Setting "+vv_VarName+'='+str(vv_NewVarValue))
    vSysLogInfo1 += 'Full External Variable List (after substitutions from vCommand):\n'
    for vv_vname in vv_myvar0:
        vv_value1 = vv_myvar0[vv_vname].strip('"').strip("'")
        vSysLogInfo1 += '  '+vv_vname+' = '+vv_value1
        if os.path.isfile(vv_value1):
            vSysLogInfo1 += ' (modified '+modification_date(vv_value1)+')'
        vSysLogInfo1 += '\n'
    vLog(vSysLogInfo1)


## A few useful molecular Biology Routintes

def vFastAToDict(Fn, upper=False, lower=False, stripN=False, idwords=''):
    ''' takes a FastA file or file name and returns a dictionary where the keys are sequence ids and the values are sequend
        input parameters are case (all lower or all upper output), whether to remove N's
        and a variable "idwords" which determines the relationship between the id line in the fastA file
        'e.g. '>chrI mouse first chromosome' and the id used in the dictionary.
        As a default we just use the first word of the id line (which in this case would be chrI.
        Another option is idwords='all', which would be the full 'chrI mouse first chromosome'
        in this case.  Additionally an arbitrary set of words can be chosen (e.g.
        idwords=(0,1,3) would assign the id 'chrI mouse chromosome'''
    if not(type(Fn)==str):
        F = Fn
        vLogOpeningFile(Fn.name,{'status':'PreDefinedFileObject'})
    else:
        if Fn.endswith('.zip'):
            ZipFile1 = zipfile.ZipFile(Fn)  ##.readline()
            vLogOpeningFile(Fn,{'status':'ZipArchiveForFastAtoDict'})
            F0 = []
            for zfn1 in ZipFile1.namelist():
                if zfn1[0]!='_':
                    F0.extend(ZipFile1.open(zfn1,mode='r'))
        elif Fn.endswith('.gz') or Fn.endswith('.gzip'):
            F0 = [gzip.open(Fn, mode='rt'),]
            vLogOpeningFile(Fn,{'status':'gzipFileForFastAtoDict'})
        else:
            F0 = [open(Fn, mode='rt'),]
            vLogOpeningFile(Fn,{'status':'UncompressedTextFileForFastAtoDict'})
        F = chain(*(F0+['>x',]))
    D = {}
    S = []
    if type(idwords)==int:
...
def vtdvToDict(FileName,idcolumn=0, idUpstreamContext='', idDownstreamContext='', delimiter='\t'):
    '''Returns a list of headers, a dictionary of values, and a list of column sums from a tabular data file.
        Keys are either the first column (default) or a tuple of user specified columns.
        idUpstreamContext and idDownstreamContext strings that can be used to describe the context
        of the feature name in a more complex columnar entry
            Example:
            If the third column contains a long string with the gene name embedded in the format ...gene_name=MyGene;...
            setting
                idcolumn=2 idUpstreamContext='gene_name=' idDownstreamContext=';'
            will take the relevant column starting after the first occurence of 'gene_name='
            and ending just before the next ';' as the gene id 
        Values are lists of column values.
        Values that are convertible to integer, float, or kept as text in that order
        Returns HeaderList,ValueDictionary,SumList'''
    D = {} ## dictionary with keys being some form of unique ID for a gene or segment, values are lists derived from the relevant data line
    H = [] ## lists of headers from the last non-data line in the input file (generally the header line
    S = {} ## sums of individual columns (keys are column number values are sum (as integer of float depending on the column entries)
    tdvFile = vOpen(FileName, mode='rt')
    colnum = 0
    if type(idcolumn) in (int,float):
        idcolumn = (idcolumn,)
    for L0 in tdvFile:
        L1 = L0.strip()
        if not(L1) or (L1[0] in ('#>!')): continue
        L2 = L1.split(delimiter)
        colnum = max(colnum,len(L2))
        L3 = []
        vn1 = 0
        for v1 in L2:
            try:
        ...
        if vn1==0 and not(H):
            ...
            for id1 in idcolumn:
                newid = L3[id1]
                if idUpstreamContext and type(newid)==str:
                    newid = newid.split(idUpstreamContext,1)[-1]
                if idDownstreamContext and type(newid)==str:
                    newid = newid.split(idDownstreamContext,1)[0]
                key1.append(newid)
            if len(key1)==1:
                key1 = key1[0]
            else:
            ...
            D[key1] = L3
    if not(H) and L3:
        H = [str(FileName)+'_column_'+str(i) for i in range(len(L2))]
    S1 = []
...
def vrenderPDF(fspec):
    from glob import glob
    MyFiles =  glob(fspec)
    if not(MyFiles):
        print('No files found for specification '+fspec+'; No conversions performed.')
    for fpsvg in MyFiles:
        fppdf = fpsvg[:-4]+'.pdf'
        print('Starting Conversion of '+fpsvg+' to pdf as '+fppdf)
        d1svg = svg2rlg(fpsvg)
        renderPDF.drawToFile(d1svg,fppdf)
        print('Completed Conversion of '+fpsvg+' to pdf as '+fppdf)

if __name__=='__main__':   ##just display some stuff
    if len(sys.argv)>1 and (sys.argv[1].lower().split('=')[0] in ('rendersvg', 'svg2pdf','svgtopdf', 'vrenderpdf')):
        if len(sys.argv) == 2 and not(('=') in sys.argv[1]):
            vrenderPDF('*.svg')
        elif len(sys.argv)==2 and ('=' in sys.argv[1]):
            vrenderPDF(sys.argv[1].split('=',1)[-1].strip())
        else:
            vrenderPDF(sys.argv[2].strip().strip('"'+"'").strip())
    else:
        for i1 in range(500): 
        ...
        vlegend(text='To apply VSG, import VSG_Module into your own drawing script (e.g. "import VSG_ModuleXX as vsg", or "from VSG_ModuleXX import *")')
        ...
        vlegend(text='Copywrite 2009-2023 Andrew Fire and Stanford University, All Rights Reserved', font='Georgia 8')
        if len(sys.argv)>1 and 'color' in sys.argv[1].lower():
            cdcxmin1,cdcymin1=VSG.xmin+30,VSG.ymin-20
            for cdn1,cdc1 in enumerate(sorted(list(vhue.keys()))):
                row=cdn1//42; col=cdn1%42
                vrect(xc=col*35+cdcxmin1,yc=-row*20+cdcymin1,xr=20,yr=10,fill=vhue[cdc1],stroke=black,strokewidth=1)    
                vtext(xc=col*35+cdcxmin1,yc=-row*20+cdcymin1,text=cdc1,font='DejaVuBold 4',color=black)
